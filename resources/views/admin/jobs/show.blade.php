                                ...
                                <tbody>
                                    @foreach ($job->proposals as $key => $proposal)
                                        <tr data-entry-id="{{ $proposal->id }}">
                                            <td>
                                                @php
                                                    $candidateId = $proposal->teacher_id;
                                                    $candidateName = DB::select('select * from users where id =' . $candidateId);
                                                    // dd($candidateName[0]);
                                                @endphp
                                                @foreach ($candidateName as $cname)
                                                    <a
                                                        href="{{ route('teacherprofile', $cname->id) }}">{{ $cname->name }}</a>
                                                ...
                                                @if ($proposal->attachments)
                                                    @foreach ($proposal->attachments as $key => $media)
                                                        <a class="badge badge-success"
                                                            href="{{ route('admin.proposals.downloadMedia', $media) }}"
                                                            target="_blank">
                                                            view file
                                                        </a>
                                                    @endforeach
